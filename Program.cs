        ...
        static async Task ChooseLanguage()
        {
            ...
            switch (languages)
            {
                ...
                case "Typescript":
                    if (!AnsiConsole.Confirm("Use Default output ?"))
                    {
                        var projectName = AnsiConsole.Ask<string>("What would you like to call your bot: ");
                        var projectPath = AnsiConsole.Ask<string>("Path: ");
                        var validation = IsValidPath(projectPath);
                        if (validation)
                        {
                            await AnsiConsole.Progress()
                                ...
                                {
                                    var dirTask = ctx.AddTask("Creating Directory");
                                    await CreateDirectory(projectPath);
                            ...
                            await AnsiConsole.Progress()
                                .StartAsync(async ctx =>
                                {
                                    var genTask = ctx.AddTask("Generating Typescript Code");
                                    await SourceCodeGenerator.Instance.GenerateTypescriptCode(projectPath, projectName);

                                    while (!ctx.IsFinished)
                                    {
                                        genTask.Increment(1.5);
                                    }
                                });
                    ...
                    else
                    {
                        var defaultPath = CreateDefaultDirectory();
                        var projectName = AnsiConsole.Ask<string>("What would you like to call your bot: ");
                        AnsiConsole.WriteLine(defaultPath);
                        await AnsiConsole.Progress()
                                .StartAsync(async ctx =>
                                {
                                    var task1 = ctx.AddTask("Generating Typescript Code");
                                    await SourceCodeGenerator.Instance.GenerateTypescriptCode(defaultPath, projectName);
